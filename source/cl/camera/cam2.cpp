#include <cl/math/vec2.h>
#include <cl/math/vec3.h>
#include <cl/math/mat4.h>
#include <cl/math/mat4_affine.h>
...
cam2_t cam2_new() {
    ...
    cam.position = vec2();
    cam.right = vec2(1.0f, 0.0f);
    cam.up = vec2(0.0f, 1.0f);
    cam.world_up = vec2(0.0f, 1.0f);
...
void cam2_compute_proj(cam2_t& cam, s32 viewport_x, s32 viewport_y) {
    ...
    mat4_scale(cam.projection, cam.projection, vec3(cam.scale / (f32)viewport_x, cam.scale / (f32)viewport_y, 1.0f));
}

void cam2_compute_view(cam2_t& cam) {
    mat4_ident(cam.view);
    mat4_translate(cam.view, cam.view, vec3(-cam.position.x, -cam.position.y, 0.0f));
}
